...
{
    Log("+++ InputManager::InputManager ...\n");

    m_bRotated90 = false;
    m_fInputRate = (GameManager::Instance().DeviceType() == GameManager::DEVICE_1ST_GEN) ? 0.1f : 0.05f;
    m_Timer.Start();

    Log("+++ InputManager::InputManager correcto\n");
}
...
InputManager::~InputManager()
{
    ...
    Log("+++ InputManager::~InputManager destruido\n");
}
...
void InputManager::HandleTouch(UITouch* touch, UIView* view)
{
    ...
    if (m_bRotated90)
    {
        if (GameManager::Instance().IsHomeRight())
        {
            ...
            location.y = IPHONE_SCREEN_WIDTH - temp;

            temp = previousLocation.x;
            previousLocation.x = previousLocation.y;
            previousLocation.y = IPHONE_SCREEN_WIDTH - temp;
        }
        else
        {
            float temp = location.x;
            location.x = IPHONE_SCREEN_HEIGHT - location.y;
            location.y = temp;

            temp = previousLocation.x;
            previousLocation.x = IPHONE_SCREEN_HEIGHT - previousLocation.y;
            previousLocation.y = temp;
        }
    }

    TRegionEventVector::size_type size = m_RegionEventVector.size();

    //Log("::::::: input %d %f, %f\n", touch.phase, previousLocation.x, previousLocation.y);
