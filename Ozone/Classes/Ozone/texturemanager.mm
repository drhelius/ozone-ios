...
{
    Log("+++ TextureManager::TextureManager ...\n");

    Log("+++ TextureManager::TextureManager correcto\n");
}
...
TextureManager::~TextureManager()
{
    ...
    Log("+++ TextureManager::~TextureManager destruido\n");
}
...
{
    Log("+++ TextureManager::LoadTexture Cargando textura: %s\n", pTexture->m_strName);
    ...
    if (pFile != NULL)
    {
        ...
        {
            Log("@@@ TextureManager::LoadTexture Tamaño incorrecto, versión PVR: %s.pvr\n", pTexture->m_strName);
            SafeDelete(pBuffer);
            return false;
        }
        ...
        Log("+++ TextureManager::LoadTexture tamaño %dx%d\n", pTexture->m_iWidth, pTexture->m_iHeight);
        Log("+++ TextureManager::LoadTexture Textura PVR cargada correctamente: %s.pvr\n", pTexture->m_strName);

        return true;
    }
    else
    {
        Log("@@@ TextureManager::LoadTexture No se pudo cargar la versión PVR: %s.pvr\n", pTexture->m_strName);
        ...
        if (spriteImage)
        {
            ...
            if (mipmaps)
            {
                Log("+++ TextureManager::LoadTexture genrando mipmaps...\n");
            ...
            Log("+++ TextureManager::LoadTexture tamaño %dx%d\n", pTexture->m_iWidth, pTexture->m_iHeight);
            Log("+++ TextureManager::LoadTexture Textura PNG cargada correctamente: %s.png\n", pTexture->m_strName);
            return true;
        }
        else
        {
            Log("@@@ TextureManager::LoadTexture Imposible cargar textura: %s\n", pTexture->m_strName);
            Log("@@@ TextureManager::LoadTexture Tratando de cargar missing texture\n");
            ...
            if (spriteImage)
            {
                ...
                Log("+++ TextureManager::LoadTexture Textura missing_texture.png cargada correctamente\n");
                return true;
            }
            else
            {
                Log("@@@ TextureManager::LoadTexture Imposible cargar missing texture: missing_texture.png\n");
...
GLboolean TextureManager::GenMipMap2D(GLubyte *src, GLubyte **dst, int srcWidth, int srcHeight, int *dstWidth, int *dstHeight, int level)
{
    ...
    *dst = (GLubyte*) malloc(sizeof(GLubyte) * texelSize * (*dstWidth) * (*dstHeight));
    ...
    Log("+++ TextureManager::GenMipMap2D nivel %d: %dx%d\n", level, *dstWidth, *dstHeight);
...
{
    Log("+++ TextureManager::GetTexture Buscando %s\n", strTextureName);
    ...
    {
        Log("+++ TextureManager::GetTexture Ya existía\n");
    ...
    else
    {
        ...
        if (LoadTexture(temp, mipmaps))
        {
            ...
            if (result->second)
            {
                ...
                Log("+++ TextureManager::GetTexture Se ha insertado la nueva textura\n");
                return temp;
            }
            else
            {
                ///--- ya exist√≠a??? nunca debe entrar aqui
                Log("@@@ TextureManager::GetTexture ERROR GRAVE @@@\n");
            }
        }
        else
        {
            ///--- no se pudo cargar la textura
            Log("@@@ TextureManager::GetTexture ERROR No se pudo cargar la textura %s\n", strTextureName);
        }
...
bool TextureManager::UnloadTexture(const char* strTextureName)
{
    ...
    {
        Log("+++ TextureManager::UnloadTexture Eliminando textura: %s\n", itor->second->m_strName);
    ...
    {
        Log("@@@ TextureManager::UnloadTexture La textura NO existía\n");
        return false;
    }
}
...
int TextureManager::UnloadAll(void)
{
    ...
    {
        Log("+++ TextureManager::UnloadAll Eliminando textura: %s\n", i->second->m_strName);
