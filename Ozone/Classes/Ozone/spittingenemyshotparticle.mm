...
/* 
 * File:   spittingenemyshotparticle.cpp
 * Author: nacho
 * 
...
SpittingEnemyShotParticle::SpittingEnemyShotParticle(void) : Particle() { }

//////////////////////////
//////////////////////////

void SpittingEnemyShotParticle::Init(void)
{
    m_MainRenderObject.Init(MeshManager::Instance().GetMeshFromFile("game/entities/10b_ent"),
...
void SpittingEnemyShotParticle::Update(Timer* timer)
{
    if (!m_bDead)
    {
...
void SpittingEnemyShotParticle::getWorldTransform(btTransform &worldTransform) const
{
...
void SpittingEnemyShotParticle::setWorldTransform(const btTransform &worldTransform)
{
...
void SpittingEnemyShotParticle::ContactWithNPC(NPC* theNPC, const float impulse, Timer* timer, int additionalDataThis, int additionalDataOthers)
{
    m_bDead = true;
}
...
void SpittingEnemyShotParticle::ContactWithPlayer(Player* thePlayer, const float impulse, Timer* timer)
{
    m_bDead = true;
    ...
    ParticleManager::Instance().AddExplosion(PARTICLE_ENEMY, m_vPosition, 0.3f, 0.6f, Vec3(0, 0, 0), 6, 0.0f);
}

//////////////////////////
//////////////////////////

void SpittingEnemyShotParticle::ContactWithPlayerNuke(Player* thePlayer, const float impulse, Timer* timer, int additionalData)
{
    ParticleManager::Instance().AddExplosion(PARTICLE_ENEMY, m_vPosition, 0.3f, 0.6f, Vec3(0, 0, 0), 6, 0.0f);
...
void SpittingEnemyShotParticle::Enable(void)
{
    Particle::Enable();
    m_MainRenderObject.SetColor(1.0f, 1.0f, 1.0f, 1.0f);
...
void SpittingEnemyShotParticle::Disable(void)
{
    Particle::Disable();
