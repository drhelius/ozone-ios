...
/* 
 * File:   movingcrosssmallenemy.cpp
 * Author: nacho
 * 
...
btCollisionShape* MovingCrossSmallEnemy::m_pCrossHorizontalShape = NULL;
btCollisionShape* MovingCrossSmallEnemy::m_pCrossVerticalShape = NULL;
btCompoundShape* MovingCrossSmallEnemy::m_pCrossCompoundShape = NULL;

MovingCrossSmallEnemy::MovingCrossSmallEnemy(void) : Enemy()
{
    if (m_pCrossHorizontalShape == NULL)
    {
        btScalar mass = 10.0f;
        btVector3 inertia(0, 0, 0);
        m_pCrossHorizontalShape = new btBoxShape(btVector3(1.2f, 0.4f, 0.1f));
    ...
    if (m_pCrossVerticalShape == NULL)
    {
        btScalar mass = 10.0f;
        btVector3 inertia(0, 0, 0);
        m_pCrossVerticalShape = new btBoxShape(btVector3(0.1f, 0.4f, 1.2f));
...
MovingCrossSmallEnemy::~MovingCrossSmallEnemy()
{
...
void MovingCrossSmallEnemy::Init(short posX, short posY, u8 rotation, short width, short height, char* szScript)
{
    m_bReverse = false;
    ...
    m_MainRenderObject.Init(MeshManager::Instance().GetMeshFromFile("game/entities/17_ent"),
...
void MovingCrossSmallEnemy::Update(Timer* timer)
{
    float rot = fmod(timer->GetActualTime() * MAT_ToRadians(CROSS_ENEMY_SMALL_ROTATION_RATE), TWOPIf);
...
void MovingCrossSmallEnemy::getWorldTransform(btTransform &worldTransform) const
{
...
void MovingCrossSmallEnemy::setWorldTransform(const btTransform &worldTransform)
{
...
void MovingCrossSmallEnemy::ContactWithPlayer(Player* thePlayer, const float impulse, Timer * timer)
{
