...
/* 
 * File:   inlinethrowingenemyshotparticle.cpp
 * Author: nacho
 * 
...
InlineThrowingEnemyShotParticle::InlineThrowingEnemyShotParticle(void) : Particle()
{
}
...
void InlineThrowingEnemyShotParticle::Init(void)
{
    m_MainRenderObject.Init(MeshManager::Instance().GetMeshFromFile("game/entities/02b_ent"),
...
void InlineThrowingEnemyShotParticle::Update(Timer* timer)
{
    if (!m_bDead)
    {
...
void InlineThrowingEnemyShotParticle::getWorldTransform(btTransform &worldTransform) const
{
...
void InlineThrowingEnemyShotParticle::setWorldTransform(const btTransform &worldTransform)
{
...
void InlineThrowingEnemyShotParticle::ContactWithNPC(NPC* theNPC, const float impulse, Timer* timer, int additionalDataThis, int additionalDataOthers)
{
    m_bDead = true;
}
...
void InlineThrowingEnemyShotParticle::ContactWithPlayer(Player* thePlayer, const float impulse, Timer* timer)
{
    m_bDead = true;
...
void InlineThrowingEnemyShotParticle::ContactWithPlayerNuke(Player* thePlayer, const float impulse, Timer* timer, int additionalData)
{
    ParticleManager::Instance().AddExplosion(PARTICLE_ENEMY, m_vPosition, 0.3f, 0.8f, Vec3(0, 0, 0), 6, 0.0f);
...
void InlineThrowingEnemyShotParticle::Enable(void)
{
    Particle::Enable();
    if (m_bUseColor)
    {
...
void InlineThrowingEnemyShotParticle::Disable(void)
{
    Particle::Disable();
