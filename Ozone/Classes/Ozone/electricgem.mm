...
ElectricGem::ElectricGem(void) : Gem()
{
    m_fFadeOut = 0.0f;
...
void ElectricGem::Init(short posX, short posY, u8 rotation, short width, short height, char* szScript)
{
    m_MainRenderObject.Init(MeshManager::Instance().GetMeshFromFile("game/items/15_item"),
...
void ElectricGem::ContactWithPlayer(Player* thePlayer, const float impulse, Timer* timer)
{
    if (m_fFadeOut == 0.0f)
    {
        thePlayer->AddAmmo(ELECTRIC_AMMO, 40.0f);
        m_pFadeInterpolator->Reset();
        InterpolatorManager::Instance().Add(m_pFadeInterpolator, false, timer);

        ParticleManager::Instance().AddExplosion(PARTICLE_ITEM, m_vPosition, 0.8f, 1.0f, Vec3(0, 0, 0), 8, 0.2f, 0.25f, 0.62f, 1.0f);
...
void ElectricGem::Update(Timer* timer)
{
    if (m_bEnable)
    {
