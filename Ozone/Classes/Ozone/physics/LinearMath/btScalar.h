...
#if (defined (__APPLE__) && defined (__i386__) && false)
	#define BT_USE_SSE
	#include <emmintrin.h>
...
#define PIOVERTWOf__	(3.1415926535f / 2.0f)
#define M_1_PIf__	(1.0f / 3.1415926535f)

inline float MAT_abs__(float num)
{
    return(num < 0) ? -num : num;
}

inline float fast_sin__(float x)
{
    ...
    float y = (4 - 4 * MAT_abs__(x)) * x;

    y = P * (y * MAT_abs__(y) - y) + y;

    return(k & 1) ? -y : y;
}

inline float fast_cos__(float x)
{
    return fast_sin__(x + PIOVERTWOf__);
}

SIMD_FORCE_INLINE btScalar btFabs(btScalar x) { return fabsf(x); }
SIMD_FORCE_INLINE btScalar btCos(btScalar x) { /*return cosf(x);*/ return fast_cos__(x); }
SIMD_FORCE_INLINE btScalar btSin(btScalar x) { /*return sinf(x);*/ return fast_sin__(x); }
